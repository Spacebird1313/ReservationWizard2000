...
public class PlaatsToewijsWindow extends JFrame implements ActionListener
{
	...
	public PlaatsToewijsWindow(ReservationWizard2000 RW2000, Voorstelling voorstelling, Reservatie reservatie, int aantalPlaatsen, JFrame parent)
	{
		this.RW2000 = RW2000;
		this.parent = parent;
		...
		this.geselecteerdePlaatsen = new ArrayList<Plaats>();
		...
		beschikbaarLabel = new JLabel("<html>Beschikbaar: <font color=\"green\">" + this.huidigeVoorstelling.zaalOpvragen().beschikbaarheidOpvragen() + "</html>");
		beschikbaarLabel.setFont(font);
		beschikbaarLabel.setBounds(9, 22, 265, 25);
		
		bezetLabel = new JLabel("<html>Bezet: <font color=\"red\">" + (this.huidigeVoorstelling.zaalOpvragen().aantalPlaatsenOpvragen() - this.huidigeVoorstelling.zaalOpvragen().beschikbaarheidOpvragen()) + "</html>");
		bezetLabel.setFont(font);
		bezetLabel.setBounds(294, 22, 227, 25);
		
		reservatieNaamLabel = new JLabel("Naam: " + this.huidigeReservatie.reservatieNaamOpvragen());
		...
		aantalPlaatsenLabel = new JLabel("<html>Plaatsen gereserveerd: 0 van " + this.aantalPlaatsen + "</html>"); 
		aantalPlaatsenLabel.setFont(font);
		aantalPlaatsenLabel.setBounds(9, 205, 505, 25);
		
		bedragLabel = new JLabel("Te betalen bedrag: € 0.0");
		...
		this.zaalweergavenWindow.zaalPreviewGenereren(huidigeVoorstelling, null, false);
	...
	public void automatischPlaatsToewijzen()
	{
		Plaats[] plaatsen = RW2000.automatischePlaatsSelector(huidigeVoorstelling, aantalPlaatsen);
		Reservatie autoReservatie = new Reservatie("autoReservatie", -1);
		for(int i = 0; i < plaatsen.length; i++)
		{
			this.geselecteerdePlaatsen.add(plaatsen[i]);
			autoReservatie.voegPlaatsToe(plaatsen[i]);
		}
		
		this.zaalweergavenWindow.zaalPreviewGenereren(huidigeVoorstelling, autoReservatie, false);
	...
	public void selectieWijzigen(int[] nieuweIndex)
	{
		int i = 0;
		int geselecteerd = 0;
	...
	private void reservatieBevestigd()
	{
		...
		this.huidigeVoorstelling.reservatieToevoegen(huidigeReservatie);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		{
			JOptionPane pane = new JOptionPane("Wenst u deze reservatie te annuleren?", JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_OPTION);
